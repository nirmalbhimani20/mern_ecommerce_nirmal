...
export const fetchMerchant = (n, v) => {
  return async (dispatch, getState) => {
    try {
      dispatch(setMerchantLoading(true));

      const reposnse = await axios.get(`${API_URL}/merchant`, {
        params: {
          page: v ?? 1,
          limit: 20
        }
      ...
      dispatch({
        ...
        payload: { totalPages, currentPages, count }
      });
    } catch (error) {
      handleError(error, dispatch);
    } finally {
...
export const searchmerchant = filter => {
  return async (dispatch, getState) => {
    try {
      dispatch(setMerchantLoading(true));
...
export const disableMerchant = (merchant, value, search, page) => {
  return async (dispatch, getState) => {
    try {
      ...
      if (search)
        return dispatch(searchmerchant({ name: 'merchant', value: search }));
      dispatch(fetchmerchant('merchant', page));
    } catch (error) {
      handleError(error, dispatch);
    }
  };
